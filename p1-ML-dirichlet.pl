...
$num_terms = 41802513; #the number of terms in database0
...
while($query_i < 25){
       ...
       while(<FH>){
              ...
              #build up the maximum likelihood with DIRICHLET SMOOTHING matrix
              ...
              if ($total_f >0){
                $index_i = 4;
                $mui = 288;  # the constant for dirichlet smoothing
                while ($index_i < $doc_num*3+3){
                        $freq = $temp[$index_i+2];
                        $docid = $temp[$index_i];
                        $doclen = $temp[$index_i+1];
                        $dirich = log($doclen/($doclen+$mui)*$freq/$doclen+$mui/($doclen+$mui)*$total_f/$num_terms);
                        if(exists $score_hash->{$docid}->{$line}){
                           $score_hash->{$docid}->{$line} +=$jk;
                        }
                        else{
                             $score_hash->{$docid}->{$line} =$jk;
                        }
              ...
              $background_p = log($mui/($doclen+$mui)*$total_f/$num_terms);
              $back_p{$line} = $background_p;
